...
- ES6의 객체 디스트럭처링 할당은 객체의 각 프로퍼티를 객체로부터 추출하여 1개 이상의 변수에 할당한다. 이때 객체 디스트럭처링 할다으이 대상(할당문의 우변)은 객체이어야 하며, **할당 기준은 프로퍼티 키다.**
- 즉, 순서는 의미가 없으며 선언된 변수 이름과 프로퍼티 키가 일치하며 할당된다.
...
- 객체 디스트럭처링 할당을 위한 변수에 Rest 파라미터와 유사하게 **Rest 요소 ...**을 사용할 수 있다.
- Rest 요소는 Rest 파라미터와 마찬가지로 반드시 마지막에 위치해야 한다.